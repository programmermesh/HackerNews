...
export class HackernewsApiService {
  ...
  getFeed(feedType): Observable<any> {
    return this._api
      ...
      .pipe(
        tap(
          (data) => (
            (this.cacheFeed = data), (this.cacheFeedSize = data.length)
          )
        )
      );
  }
    ...
    return this.getFeed(feedType)
      .pipe(
        map((data) =>
          data.slice(
            startCount - 1,
            Math.min(startCount + 4, this.cacheFeedSize)
          )
        )
      )
  ...
  getCommentItemInRange(feedType, startCount, endCount): Observable<any> {
    return this.getCommentTree(feedType)
      .pipe(
        map((data) =>
          data.slice(
            startCount - 1,
